...
const CORE = ['/', '/demo/', '/privacy/', '/terms/', '/not-found.html', '/offline.html', '/manifest.webmanifest', '/icon.svg', '/icon-192.png', '/icon-512.png', '/icon-maskable-512.png', '/apple-touch-icon.png', '/assets/name-tap-hero-720.webp', '/assets/name-tap-hero-1200.webp', '/assets/name-tap-social.webp'];
  ...
  if (request.mode === 'navigate') {
    event.respondWith((async () => {
      try {
        ...
        if (!known.has(url.pathname)) return (await cache.match('/not-found.html')) ?? (await cache.match('/offline.html'));
